...
# IP
  ...
  foreach my $nodename (keys(%{$description->{nodes}})) {
    ...
    foreach my $interfacename (keys(%{$node->{interfaces}})) {
      ...
      # 'ifconfig', $ident, 'up');
      @cmd = ( qw(ip netns exec), $nodename, 'ip', 'link', 'set', $ident, 'up');
      ...
      if ($interface->{ip}) {
        @cmd = (
          qw(ip netns exec),
          $nodename,
          'ip', 'addr', 'add', $interface->{ip}, 'dev', $ident
          # 'ifconfig', $ident, $interface->{ip}
        );
      ...
      if ($interface->{ipv6}) {
        @cmd = (
          qw(ip netns exec),
          $nodename,
          # 'ifconfig', $ident, 'inet6', 'add', $interface->{ipv6}
          'ip', 'addr', 'add', $interface->{ipv6}, 'dev', $ident
        );
...
sub list {
  ...
  foreach my $nodename (keys(%{$description->{nodes}})) {
    print STDERR "\n\n\nNamespace $nodename ip config and routing table.\n";
